...
def triangulate(output, uri, **request):
    if request["method"] == "POST":
        ...
        for image in poses:
            ...
            intrinsics = np.matrix(
                [float(x) for x in tags["IntrinsicsMatrix"].split("\\")]
            ).reshape((3, 3))
            rotation = np.matrix(
                [float(x) for x in tags["RotationMatrix"].split("\\")]
            ).reshape((3, 3))
            trans = np.matrix(
                [float(x) for x in tags["TranslationMatrix"].split("\\")]
            ).reshape((3, 1))
            dist_coeffs = np.matrix(
                [float(x) for x in tags["DistortionCoefficients"].split("\\")]
            ).reshape(1, -1)
    ...
    if request["method"] == "GET":
        ...
        position = np.array(
            [
                ...
                float(request["get"]["y"]),
                float(request["get"]["z"]),
                float(request["get"]["w"]) if "w" in request["get"] else 1.0,
            ]
        )
